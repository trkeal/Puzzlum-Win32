...
sub Timothy_memory_dump(Timothy_db() as Timothy_node_type, ret() as string, label as string, byref index as uinteger, byval octet as uinteger, byref biet as uinteger)
    ...
    redim as string dump(0 to 0,0 to 1)
...
sub Timothy_memory_dump_loop(Timothy_db() as Timothy_node_type, ret() as string, label as string, byref index as uinteger, byval octet as uinteger, byref biet as uinteger)
    ...
    if Timothy_db(index).cell.written and biet=0 and len(label2)>0 then
        redim preserve ret(lbound(ret,1) to ubound(ret,1)+1,0 to 1)
    ...
    for f=0 to 3
        if Timothy_db(index).fork(f).written then
            ...
            nextlabel=left(label,len(label)-1)+chr((asc(right(label,1)) and (&hff shl (biet shl 1)) and &hff) or ((f and &h3) shl (biet shl 1)))
...
sub Timothy_memory_queue(Timothy_db() as Timothy_node_type, queue() as Timothy_queue_type, label as string, byval request as uinteger, byval waccess as uinteger, byref index as uinteger, byval octet as uinteger, byref biet as uinteger, byref f as uinteger)
    dim as uinteger octet2=0
    redim as Timothy_queue_type queue(0)
    Timothy_memory_queue_loop Timothy_db(), queue(), label, request, waccess, index, octet, biet, f
end sub

sub Timothy_memory_queue_loop(Timothy_db() as Timothy_node_type, queue() as Timothy_queue_type, label as string, byval request as uinteger, byval waccess as uinteger, byref index as uinteger, byval octet as uinteger, byref biet as uinteger, byref f as uinteger)
    dim as uinteger tmp=0,nextindex=0,nextoctet=0,nextbiet=0,nextfork=0
    dim as string nextlabel=space(0)
    if biet=3 then
        nextoctet=nextoctet+1
        nextbiet=0
        nextlabel=nextlabel+chr(0)
    else
        nextoctet=octet
        nextbiet=biet+1
        nextlabel=label
    endif
    if Timothy_db(index).cell.written and biet=0 and len(label)>0 then
        redim preserve queue(lbound(queue,1) to ubound(queue,1)+1)
        queue(ubound(queue,1)).label=label
        queue(ubound(queue,1)).value=Timothy_db(index).cell.value
        queue(ubound(queue,1)).octet=octet
        queue(ubound(queue,1)).biet=biet
        queue(ubound(queue,1)).fork=f
    ...
    for nextfork=0 to 3
        if Timothy_db(index).fork(nextfork).written then
            nextindex=Timothy_db(index).fork(nextfork).address
            nextlabel=left(label,len(label)-1)+chr((asc(right(label,1)) and (&hff shl (biet shl 1)) and &hff) or ((nextfork and &h3) shl (biet shl 1)))
            redim as Timothy_queue_type queue2(0)
            Timothy_memory_queue_loop Timothy_db(),queue2(),nextlabel,request,waccess,nextindex,nextoctet,nextbiet,nextfork
            for tmp=lbound(queue2,1)+1 to ubound(queue2,1)
                redim preserve queue(lbound(queue,1) to ubound(queue,1)+1)
                queue(ubound(queue,1))=queue2(tmp)
            next
        endif
    next
end sub

sub Timothy_memory_queue_load(queue() as Timothy_queue_type, filename as string, byval request as uinteger, byval waccess as uinteger)
    dim as uinteger mode=freefile, tmp=0, ct=0
    ...
    while not eof(mode)
        ...
        if tmp>0 then
            ...
            redim preserve queue(0 to ct-1)
            queue(ubound(queue,1)).label=label
            queue(ubound(queue,1)).value=value
            queue(ubound(queue,1)).request=request
            queue(ubound(queue,1)).waccess=waccess
...
sub Timothy_memory_queue_save(queue() as Timothy_queue_type, filename as string, byval request as uinteger, byval waccess as uinteger)
    dim as uinteger mode=freefile, tmp=0, ct=0
    dim as string label=space(0), value=space(0), dat=space(0)
    open filename for output as #mode
    for tmp=lbound(queue,1) to ubound(queue,1)
        print #mode,chr(34)+Timothy_codec_tag(queue(tmp).label,TIMOTHY_CODEC_ENCODE)+chr(34)+"="+chr(34)+Timothy_codec_tag(queue(tmp).value,TIMOTHY_CODEC_ENCODE)+chr(34)
    next
    close #mode
end sub

sub Timothy_memory_queue_exec(Timothy_db() as Timothy_node_type, queue() as Timothy_queue_type, waccess as uinteger)
    dim as uinteger index=0
    for index=0 to ubound(queue,1)
        queue(index).ret=Timothy_memory(Timothy_db(),queue(index).label,queue(index).value, _
            queue(index).request,waccess and queue(index).waccess, _
            queue(index).fail,queue(index).index,queue(index).octet,queue(index).biet)
    next
end sub

sub Timothy_memory_queue_push(queue() as Timothy_queue_type, byval label as string, byval value as string, request as uinteger, waccess as uinteger)
    redim preserve queue(0 to ubound(queue,1)+1)
    queue(ubound(queue,1)).label=label
    queue(ubound(queue,1)).value=value
    queue(ubound(queue,1)).request=request
    queue(ubound(queue,1)).waccess=waccess
